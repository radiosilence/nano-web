...
fn bench_mime_detection(c: &mut Criterion) {
    let paths = vec![
        "/test.html", "/test.css", "/test.js", "/test.png", 
        "/test.jpg", "/test.svg", "/test.woff2", "/test.json",
...
fn create_test_route(content: Vec<u8>) -> routes::Route {
    ...
    let compressed = compression::CompressedContent::new(content, mime_config.is_compressible).unwrap();
